...
let rec compile env = function
  | [] -> env, []
  | instr :: code' ->
    let is_register = function | R _ -> true | _ -> false in
    let suffix = function
      | "<"  -> "l"
      | ">"  -> "g"
      | "<=" -> "le"
    ...
    let env, asm =
      ...
      | LD x ->
        ...
        in env, begin match s with
                  | S _ -> [Mov (M (env#loc x), eax); Mov (eax, s)]
                  | R _ -> [Mov (M (env#loc x), s)]
                  | _   -> failwith "WUT?!"
                end
      | ST x ->
      ...
      | CJMP (op, label) ->
          let s, env = env#pop
          in env, [Binop ("cmp", L 0, s); CJmp (op, label)]
      | BINOP op ->
          ...
          env#push y, match op with
            | "+" | "*" | "-" ->
              if is_register y
              then [Binop (op, x, y)]
              else [Mov (y, eax); Binop (op, x, eax); Mov (eax, y)]
            | "/" | "%" ->
              [
                Mov (y, eax);
                Cltd;
                IDiv x;
                Mov ((match op with | "/" -> eax | "%" -> edx), y)
              ]
            | "<" | ">" | "<=" | ">=" | "==" | "!=" ->
              [
                Mov (L 0, eax);
